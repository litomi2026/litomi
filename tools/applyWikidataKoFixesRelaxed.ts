...
async function main() {
  ...
  status.phase = 'wikidata_ko_relaxed'
  ...
  for (const [key, t] of Object.entries(dict)) {
    ...
    // Reduce false matches for extremely short/common English tokens.
...
async function wikidataGetLabels(ids: string[], timeoutMs: number): Promise<Map<string, { en?: string; ko?: string }>> {
  ...
  const out = new Map<string, { en?: string; ko?: string }>()
  for (const [id, ent] of Object.entries(data.entities)) {
    const labels = ent.labels ?? {}
    out.set(id, { en: labels.en?.value, ko: labels.ko?.value })
  }
  return out
}
