...
```zsh
kubectl create namespace litomi-stg --dry-run=client -o yaml | kubectl apply -f -
kubectl create namespace litomi-prod --dry-run=client -o yaml | kubectl apply -f -
...
kubectl -n litomi-stg create secret generic litomi-backend-secret \
  --from-env-file=/tmp/litomi-backend-secret.stg.env \
  --dry-run=client -o yaml | kubectl apply -f -

kubectl -n litomi-prod create secret generic litomi-backend-secret \
...
```zsh
kubectl apply -f k8s/bootstrap/root/root.yaml
