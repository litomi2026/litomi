...
export default function SobokLayout({ children }: LayoutProps<'/[locale]'>) {
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gray-50 dark:bg-gray-950">
      ...
      <div className="w-full h-dvh bg-white dark:bg-[#0a0a0c] sm:h-[min(100dvh-2rem,850px)] sm:max-w-lg sm:rounded-3xl sm:shadow-2xl sm:ring-1 sm:ring-gray-200 dark:sm:ring-white/10 relative overflow-hidden flex flex-col">
        <ChatProvider>
          <ChatRealtime />
          {children}
        </ChatProvider>
