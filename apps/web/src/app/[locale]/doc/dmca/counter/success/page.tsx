...
const copy: Record<Lang, { title: string; body: string; caseLabel: string; back: string }> = {
  ko: {
    title: '이의제기가 접수됐어요',
    body: `이의제기가 접수됐어요. 필요하면 ${DMCA_EMAIL}로 접수 번호와 함께 연락해 주세요.`,
    caseLabel: '접수 번호',
    back: '이의제기 페이지로 돌아가기',
  },
  en: {
    title: 'Counter notice submitted',
    body: `Your counter notice has been received. If you need to follow up, contact ${DMCA_EMAIL} with the case ID below.`,
    caseLabel: 'Case ID',
    back: 'Back to counter notice page',
  },
}

export default async function Page({ searchParams }: PageProps<'/[locale]/doc/dmca/counter/success'>) {
