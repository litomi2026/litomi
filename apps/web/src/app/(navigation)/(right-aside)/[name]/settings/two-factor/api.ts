...
export async function disableTwoFactor(body: DELETEV1MeTwoFactorBody) {
  ...
  const { data } = await fetchWithErrorHandling<DELETEV1MeTwoFactorResponse>(url, {
    method: 'DELETE',
    credentials: 'include',
...
export async function regenerateTwoFactorBackupCodes(body: POSTV1MeTwoFactorBackupCodesBody) {
  ...
  const { data } = await fetchWithErrorHandling<POSTV1MeTwoFactorBackupCodesResponse>(url, {
    method: 'POST',
    credentials: 'include',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
