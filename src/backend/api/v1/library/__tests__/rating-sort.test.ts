...
describe('rating-sort helpers', () => {
  test('manga id 오름차순은 manga_id 기준 오름차순 페이지네이션을 만든다', () => {
  ...
  test('manga id 내림차순은 manga_id 기준 내림차순 페이지네이션을 만든다', () => {
  ...
  test('created 정렬 커서는 createdAt timestamp를 사용한다', () => {
  ...
  test('manga id 정렬 커서는 기존 스키마 호환을 위해 updatedAt timestamp를 유지한다', () => {
    const createdAt = new Date('2025-01-02T00:00:00.000Z')
    const updatedAt = new Date('2025-01-03T00:00:00.000Z')
