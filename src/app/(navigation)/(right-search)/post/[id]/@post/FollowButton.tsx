...
export default function FollowButton({ leader }: Readonly<Props>) {
  ...
  const isFollowing = followingUserIds?.has(leader.id) ?? false
  const followMutation = useUserFollowMutation(leader.id, { currentFollowing: isFollowing })
  ...
  async function handleUnfollowSubmit(event: React.FormEvent<HTMLFormElement>) {
  ...
  const iconClassName = isPending ? 'size-4 animate-spin' : 'size-4'
  ...
  return (
    <>
      <button
        ...
        aria-pressed:border-zinc-700 aria-pressed:bg-zinc-950/80 aria-pressed:text-zinc-100 aria-pressed:shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]
      ...
      >
        {isPending ? (
          ...
          <UserPlus aria-hidden="true" className={iconClassName} />
        )}
