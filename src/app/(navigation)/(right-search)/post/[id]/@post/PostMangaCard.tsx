  ...
  return (
    <>
      {thumbnailUrl && (
        <MangaImage
          alt={manga?.title}
          className="w-20 h-28 object-cover rounded border-2 border-zinc-700 flex-shrink-0"
          src={thumbnailUrl}
        />
      )}
      <div className="flex flex-col gap-2 min-w-0 flex-1">
        <div className="flex items-start gap-2">
          <MessageCircle className="size-5 text-brand-end flex-shrink-0 mt-0.5" />
          <h3 className="font-bold text-base leading-tight line-clamp-2 break-words break-all">{manga?.title}</h3>
        </div>
