...
describe('MangaImage fallback', () => {
  test('원본 이미지는 direct src 이후 queryless .webp probe와 semantic materialize 후보를 거쳐 로컬 fallback으로 내려간다', () => {
    ...
    fireEvent.error(image)
    expect(image.getAttribute('src')).toBe(
      'https://example.com/i/v2/manga/123/original/5.webp?u=https%3A%2F%2Fsoujpa.in%2Fstart%2F123%2F123_4.webp',
    )

    fireEvent.error(image)
    expect(image.getAttribute('src')).toBe(
      'https://example.com/i/v2/manga/123/original/5.webp?u=https%3A%2F%2Fcdn.hentkor.net%2Fpages%2F123%2F5.avif',
    )
  ...
  test('첫 번째 썸네일은 queryless .webp probe 뒤에 cover materialize, direct cover, 1페이지 원본 fallback을 순서대로 사용한다', () => {
    ...
    fireEvent.error(image)
    expect(image.getAttribute('src')).toBe(
      'https://example.com/i/v2/manga/123/thumbnail/1.webp?u=https%3A%2F%2Fcdn.imagedeliveries.com%2F123%2Fthumbnails%2Fcover.webp',
    )
